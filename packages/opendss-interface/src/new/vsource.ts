...
import { ModelEnum, ScanTypeEnum, SequenceEnum } from "@/enums";
import { Bus, VSourceInput, vSourceSchema } from "@/schemas";
import { vsourceTransformer } from "@/transforms/vsource";
...
export default class VSource extends BaseElement implements VSourceInput {
    name: string;
    ...
    constructor(input: VSourceInput) {
        super();
        const values = vSourceSchema.parse(input);
        this.name = values.name;
        Object.assign(this, values);
    }

    create() {
        const type = this.circuit ? `Circuit` : `Vsource`;
        const script = [`New ${type}.${this.name}`];
        const values = vsourceTransformer.parse(this);
